...
print(f"Mean FA Memory Usage: {mean_fa:.2f} bytes")
print(f"Mean EFA Memory Usage: {mean_efa:.2f} bytes")
print(f"Mean Memory Usage Change: {mean_change_percent:.2f}%")
print("\n" + "=" * 50)

# ==================== OUTPUT DIRECTORY SETUP ====================

# Create output directory if it doesn't exist
output_dir = config.get_output_dir("sop2", "memory")
...
ax1.set_title("FA vs EFA Memory Usage Comparison")
ax1.legend()
...
# Save figure 1
output_path_1 = os.path.join(output_dir, config.OUTPUT_FILES["memory_comparison"])
...
ax2.set_title("Mean Memory Usage: FA vs EFA")
...
# Save figure 2
output_path_2 = os.path.join(output_dir, config.OUTPUT_FILES["mean_memory_comparison"])
...
with open(results_path, "w") as f:
    f.write("FA vs EFA Memory Usage Analysis Results\n")
    f.write("=" * 50 + "\n\n")
    f.write(f"Mean FA Memory Usage: {mean_fa:.2f} bytes\n")
    f.write(f"Mean EFA Memory Usage: {mean_efa:.2f} bytes\n")
    f.write(f"Mean Memory Usage Change: {mean_change_percent:.2f}%\n")
